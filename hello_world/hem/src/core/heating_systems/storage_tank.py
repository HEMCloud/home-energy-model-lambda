...
class StorageTank:
    ...
    def __init__(
        ...
        self.__temp_n = [init_temp] * self.__NB_VOL
    ...
    def demand_hot_water(self, usage_events):
        ...
        if usage_events is not None:
            ...
            for event in filtered_events:
                ...
                volume_used, temp_s3_n, energy_withdrawn, energy_unmet, rearrange = self.allocate_hot_water(event)
        ...
        for heat_source,  heat_source_data in self.__heat_source_data:
            _, setpntmax = heat_source.temp_setpnt()
    ...
    def allocate_hot_water(self, event):
        ...
        new_temp_distribution, flag_rearrange_layers = self.calculate_new_temperatures(remaining_vols)
        ...
        return volume_used, new_temp_distribution, energy_withdrawn, energy_unmet, flag_rearrange_layers
    ...
    def calculate_new_temperatures(self, remaining_vols):
        ...
        for i in reversed(range(len(self.__Vol_n))):
            ...
            temp_layer_min = self.__temp_n[i]    
            ...
            for j in range(i - 1, -1, -1):
                ...
                if available_volume > 0:
                    ...
                    if self.__temp_n[j] < temp_layer_min:
                        temp_layer_min = self.__temp_n[j]
            ...
            if needed_volume > 0:
                ...
                temp_cold_feed = self.__cold_feed.temperature(needed_volume)
    ...
    def potential_energy_input(self, temp_s3_n, heat_source, heater_layer, thermostat_layer):
        ...
        if isinstance(heat_source, SolarThermalSystem):
            ...
            if self.__heating_active[heat_source]:
                energy_potential = heat_source.energy_output_max(temp_flow)
    ...
    def thermal_losses(self,
        ...
        for i, vol_i in list(enumerate(self.__Vol_n)):
            Q_ls_n[i] = (H_sto_ls * self.__rho * self.__Cp) \
                        ...
                        * (min(temp_s7_n[i], setpntmax) - self.__temp_amb) \
        ...
        for i, vol_i in list(enumerate(self.__Vol_n)):
            ...
            if Q_x_in_adj > 0.0 and temp_s7_n[i] > setpntmax:
                #Case 2 - Temperature exceeding the set point
                temp_s8_n[i] = setpntmax
            else:
                ...
                for i in range(heater_layer, self.__NB_VOL):
                    energy_surplus \
                        += Q_h_sto_s7[i] - Q_ls_n[i] \
                         - (self.__rho * self.__Cp * self.__Vol_n[i] * setpntmax)
    ...
    def heat_source_output(self, heat_source, input_energy_adj):
        ...
        if isinstance(heat_source, ImmersionHeater):
            ...
            heat_source_output = heat_source.demand_energy(input_energy_adj, temp_flow) - primary_pipework_losses_kWh
    ...
    def _retrieve_setpnt(self, heat_source):
        ...
        setpntmin, setpntmax = heat_source.temp_setpnt()
    ...
    def testoutput(self, usage_events, volume_extracted, Q_use_W, Q_unmet_W,
        ...
        if self.__detailed_output:
            ...
            self.__detailed_results.append(
                [
                    str(self.__simulation_time.hour_of_day()),
                    str(self.__V_total),
                    str(self.__Cp),
                    str(self.__rho),
                    str(self.__cold_feed.temperature()),
                    demand,
    ...
    def draw_off_hot_water(self, volume):
        ...
        self.__temp_average_drawoff_volweighted = 0.0
        self.__total_volume_drawoff = 0.0
        self.__temp_average_drawoff = self.__cold_feed.temperature()
        self.__temp_final_drawoff = self.__cold_feed.temperature()       
        temp_ini_n = deepcopy(self.__temp_n)
        temp_s3_n = deepcopy(self.__temp_n)
        ...
        if remaining_demanded_volume > 0:
            self.__temp_average_drawoff_volweighted += remaining_demanded_volume * self.__cold_feed.temperature(remaining_demanded_volume)
        ...
        self.__temp_n = deepcopy(new_temp_distribution)
    ...
    def __primary_pipework_losses(self, input_energy_adj, setpnt_max):
        ...
        if input_energy_adj > 0.0 and self.__input_energy_adj_prev_timestep == 0.0:
            for pipework_data in self.__primary_pipework_lst:
                primary_pipework_losses_kWh \
                    += pipework_data.cool_down_loss(setpnt_max, self.temp_surrounding_primary_pipework(pipework_data))
        ...
        if input_energy_adj > 0.0:
            for pipework_data in self.__primary_pipework_lst:
                ...
                primary_pipework_losses_W \
                    = pipework_data.heat_loss(setpnt_max, self.temp_surrounding_primary_pipework(pipework_data))
    ...
    def temperature(self, volume_needed = 0.0):
        ...
        if volume_needed == 0.0:
            return self.__cold_feed.temperature()
        else:
...
class ImmersionHeater:
    ...
    def demand_energy(self, energy_demand):
        ...
        if energy_demand < 0.0:
            raise ValueError("Negative energy demand on ImmersionHeater")
