...
class HeatBatteryServiceWaterRegular(HeatBatteryService):
    ...
    def demand_energy(self, energy_demand, temp_return):
    ...
    def energy_output_max(self, temp_return):
        """ Calculate the maximum energy output of the heat_battery"""
        service_on = self.is_on()
        if not service_on:
            return 0.0

        return self._heat_battery._HeatBattery__energy_output_max(self.__controlmax.setpnt())
...
class HeatBattery:
    ...
    def __init__(self,
        ...
        self.labs_tests_rated_output = heat_battery_dict['labs_tests_rated_output']
        self.labs_tests_rated_output_enhanced = heat_battery_dict['labs_tests_rated_output_enhanced']
        
        # Charge level x losses
        # each as a proportion of the maximum as specified in inputs
        self.labs_tests_losses = heat_battery_dict['labs_tests_losses']
    ...
    def __demand_energy(
        ...
        E_out = min( energy_demand, self.__Q_out_ts * time_available ) #kWh
...
#        self.__max_output = max_output
    ...
    def __energy_output_max(self, temp_output, time_start=0.0):
        """ Calculate the maximum energy output of the heat battery, accounting
            for time spent on higher-priority services.

        Note: Call via a HeatBatteryService object, not directly.
        """
        ...
        max_output = self.__lab_test_rated_output(charge_level_qin)
        delta_charge_level = ( max_output ) * time_available / self.__heat_storage_capacity
        max_output = self.__lab_test_rated_output(charge_level_qin - delta_charge_level / 2) * time_available
