...
class BoilerServiceWaterRegular(BoilerService):
    ...
    def __init__(self,
        ...
        self.__temp_hot_water = self.__controlmax.setpnt()
    ...
    def energy_output_max(self, temp_return, time_elapsed_hp = None):
        """ Calculate the maximum energy output of the boiler"""
        service_on = self.is_on()
        if not service_on:
            return 0.0
...
class BoilerServiceSpace(BoilerService):
    ...
    def demand_energy(
        ...
        """ Demand energy (in kWh) from the boiler """
        if not self.is_on():
            if not hybrid_service_bool:
                # If not a hybrid hp system
                return 0.0
            else:
                # If hybrid hp system is off
                return 0.0, 0.0
    ...
    def energy_output_max(self, temp_output, temp_return_feed, time_start, time_elapsed_hp = None):
        ...
        return self._boiler._Boiler__energy_output_max(temp_output, time_start, time_elapsed_hp)
...
class Boiler:
    ...
    def __demand_energy(
        ...
        if energy_output_required <= 0.0 or time_available <= 0.0:
            energy_output_provided = 0.0
            fuel_demand = 0.0
            self.__energy_supply_connections[service_name].demand_energy(fuel_demand)

        blr_eff_final = self.calc_boiler_eff(
            service_type,
            temp_return_feed,
            energy_output_required,
            time_start,
    ...
    def __energy_output_max(self, temp_output, time_start=0.0, time_elapsed_hp = None):
        time_available = self.__time_available(time_start, time_elapsed_hp)
