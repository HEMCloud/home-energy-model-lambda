...
def calc_HTC_HLP(proj_dict: dict):
    ...
    def calc_heatloss(name, data):
        ...
        data['r_c'] = init_resistance_or_uvalue(name, data)
...
class Project:
    ...
    def __init__(
        ...
        if "Control" in proj_dict.keys() and "loadshifting" in proj_dict["Control"].keys():
            try:
                self.__controls["SmartApplianceControl"] = SmartApplianceControl(\
                    proj_dict["Control"]["loadshifting"]["power_timeseries"],
                    proj_dict["Control"]["loadshifting"]["weight_timeseries"],
                    proj_dict["Control"]["loadshifting"]["time_series_step"],
                    self.__simtime,
                    proj_dict["Control"]["loadshifting"]["non_appliance_demand_24hr"],
                    proj_dict["Control"]["loadshifting"]["battery24hr"],
                    self.__energy_supplies,
                    proj_dict['ApplianceGains'])
            except:
        ...
        for name, data in sorted(check_priority(proj_dict['ApplianceGains']).items(), key=lambda d: d[1]["priority"], reverse=True):
            ...
            if "Events" in data.keys() and "Standby" in data.keys():
                self.__internal_gains[name] = EventApplianceGains(
                                                 ...
                                                 self.__controls["SmartApplianceControl"] 
                                                 )
            else:
                # Convert energy supplied to appliances from W to W / m2
    ...
    def temp_external_air(self):
        """Return the external air temperature"""
    ...
    def run(self):
        ...
        for t_idx, t_current, delta_t_h in self.__simtime:
            ...
            if "SmartApplianceControl" in self.__controls.keys():
                self.__controls["SmartApplianceControl"].update_demand_buffer(t_idx)
