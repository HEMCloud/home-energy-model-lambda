    ...
    def test_hot_water_demand(self):
        ...
        for t_idx, _, _ in self.simtime:
            with self.subTest(i=t_idx):
                self.assertEqual(
                    self.mixershower.hot_water_demand(40.0, 52.0, 5.0)[0],
...
class TestInstantElecShower(unittest.TestCase):
    ...
    def test_hot_water_demand(self):
        ...
        for t_idx, _, _ in self.simtime:
            with self.subTest(i=t_idx):
                demand = self.instantelecshower.hot_water_demand(40.0, 52.0, ((t_idx+1)*6))[0]
                self.assertEqual(
                ...
                self.assertEqual(
                    self.instantelecshower.hot_water_demand(40.0, 52.0, ((t_idx+1)*6))[0],
